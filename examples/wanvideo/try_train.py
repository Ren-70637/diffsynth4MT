...
# Download models
#snapshot_download("Wan-AI/Wan2.1-T2V-1.3B", local_dir="/root/autodl-tmp/Wan2.1-T2V-1.3B")
...
def main(args):
    ...
    model_manager.load_models(
        [
            "/obs/pretrained_models/Wan-AI/Wan2.1-T2V-1.3B/diffusion_pytorch_model.safetensors",
            "/obs/pretrained_models/Wan-AI/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
            "/obs/pretrained_models/Wan-AI/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth",
        ],
    ...
    video = VideoData("/home/ma-user/code/wangzk/diffsynth4MT/ref_videos/car_turn_41.mp4", height=480, width=832)
    pipe.dit.train()
    # Text-to-video
    video = pipe(
        ...
        input_video=video,
        seed=args.seed, tiled=True, motion_transfer=args.motion_transfer,
        use_SWMF=args.use_SWMF,
        num_frames=41,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_SWMF", action="store_true", help="Use SWMF for motion transfer")
    parser.add_argument("--eval_guidance", action="store_true", help="Evaluate the time consumed on the guidance step")
    parser.add_argument("--motion_transfer", action="store_true", help="Transfer the motion or not")
