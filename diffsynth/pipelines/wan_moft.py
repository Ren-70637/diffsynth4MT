...
class WanMoftPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = WanMoftPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def get_dim_mask(self, intermediates):
        intermediates = intermediates.squeeze(0)
        f, c, h, w = intermediates.shape
        data = F.interpolate(intermediates, [64, 64], mode='bilinear').to(torch.float32)

        f,c,h,w = data.shape
        crop_size = 16
        data = data[:,:,h//2-crop_size//2:h//2+crop_size//2,w//2-crop_size//2:w//2+crop_size//2]

        data = data.reshape(f,c,crop_size,crop_size)
        ...
        data_tensor = data_tensor.permute(0,2,3,1).reshape(crop_size*crop_size,c)
    ...
    def guidance_step(self, 
        ...
        with torch.no_grad(): # disable the gradient computation to calculate reference of the ref video
            noise = self.generate_noise(latents.shape, device=latents.device, dtype=torch.float32)
            noise = noise.to(dtype=self.torch_dtype, device=self.device)
            ...
            if mode == 'AMF':
                ...
                amf = self.compute_tile_AMF(q_q,k_k,sf=sf, l=21, tau=1.0, tile=(3, 4))
            else:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    if j % interval == 0 or mode == 'AMF':
                        ...
                        if mode == 'AMF': 
                            ...
                            amf_new = self.compute_tile_AMF(q_q_new,k_k_new,sf=sf, l=21, tau=1.0, tile=(3, 4))
                            #loss = self.compute_amf_loss(amf, amf_new)
                            loss = self.compute_tile_amf_loss(amf, amf_new)
                    else:
    ...
    def moft_guidance_step(self, 
        ...
        with torch.no_grad(): # disable the gradient computation to calculate reference of the ref video
            ...
            noise = noise.to(dtype=self.torch_dtype, device=self.device)
            ref_latents = self.scheduler.add_noise(self.clean_latents, noise, timestep=timestep)
            _, inter_feats = self.dit(ref_latents, timestep=timestep, preserve_space=True, size_info=size_info, return_intermediates=True, **prompt_null, **image_emb, **extra_input)
            inter_feat = inter_feats[14]
            inter_feat = rearrange(inter_feat, "b (f h w) d -> b f d h w", f=size_info['frames'], h=size_info['tile_size'][0])
            moft = inter_feat - inter_feat.mean(dim=1, keepdim=True)  # (B, F, D_h, H, W)
            mask_dim = self.get_dim_mask(moft)
        ...
        for j in tqdm(range(total_steps)):
            ...
            with torch.enable_grad():
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    self.dit.train()
                    _, gen_inter_feats = self.dit(optimized_latents, timestep=timestep, size_info=size_info, preserve_space=True, return_intermediates=True, **detached_prompt_emb_posi_new, **image_emb, **extra_input)
                    gen_inter_feat = gen_inter_feats[14]
                    gen_inter_feat = rearrange(gen_inter_feat, "b (f h w) d -> b f d h w", f=size_info['frames'], h=size_info['tile_size'][0])
                    gen_moft = gen_inter_feat - gen_inter_feat.mean(dim=1, keepdim=True)  # (B, F, D_h, H, W)
                    gen_moft = gen_moft[:, :, mask_dim]
                    ref_moft = moft[:, :, mask_dim]
                    loss = F.mse_loss(gen_moft, ref_moft) * motion_guidance_weight
    ...
    def __call__(
        ...
        with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self.device).type, enabled=True):
            i_for_guidance = 0
            if test_latency:
            ...
            for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
                ...
                ########################################################################
                if mode in ['AMF', 'effi_AMF'] and i_for_guidance < 10:
                    if test_latency:
                ...
                ##############################################################################
                    ...
                    noise_pred_posi, inter_feats = model_fn_wan_video(
                        self.dit, motion_controller=self.motion_controller, vace=self.vace,
                        x=latents, timestep=timestep, size_info=size_info, return_intermediates=True,
                        **prompt_emb_posi, **image_emb, **extra_input,
                        **tea_cache_posi, **usp_kwargs, **motion_kwargs, **vace_kwargs,
                    )
                    if i_for_guidance < 10:
                        latents = self.moft_guidance_step(
                            latents, timestep, prompt_emb_posi, prompt_null, image_emb, extra_input, noise,
                            size_info, motion_guidance_weight=10.0
                        )
                        i_for_guidance += 1
                    if cfg_scale != 1.0:
                        noise_pred_nega = model_fn_wan_video(
